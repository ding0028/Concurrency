...
void TrafficLight::cycleThroughPhases()
{
    ...
    while (true)
    {
        long timeSinceStart = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - start).count(); 

        if (timeSinceStart >= cycleDuration)
        {           
            _currentPhase = (_currentPhase == red) ? green : red;
            _queue.send(std::move(_currentPhase));            
            //reset start and cycleDuration for next cycle
